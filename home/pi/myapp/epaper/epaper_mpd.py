...
pathbmp = '/home/pi/myapp/epaper/bmp/'
...
# Auth if password is set non False
  ...
  while(1):
    #client.idle()
    #client.send_idle()
    #state = client.fetch_idle() # legacy
    state = client.idle()
    ...
    try:
      ...
      stationChk2 = re.findall(r'\&station_name=(.*)', station_url)  # ['MY_STATION_NAME']      
      if len(station) > 0:
        station = station[0].replace('_', ' ')  # MY STATION NAME
      elif len(stationChk2) > 0:
        station = stationChk2[0].replace('_', ' ')  # MY STATION NAME
      else:
        station = ''
    except KeyError:
      station = ''
